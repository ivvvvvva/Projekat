...
int main()
{
    ...
    Shader blend("resources/shaders/blanding.vs","resources/shaders/blanding.fs");
    ...
    unsigned int transparentTexture = loadTexture(FileSystem::getPath("resources/textures/alien-22348.png").c_str());
    unsigned int diffuseMap = loadTexture(FileSystem::getPath("resources/textures/difuzna.jpg").c_str());
    unsigned int specularMap = loadTexture(FileSystem::getPath("resources/textures/spek.jpg").c_str());
    lightShader.use();
    ...
    // floor textures
    unsigned int floorDiffuseMap = loadTexture(FileSystem::getPath("resources/textures/difuzna.jpg").c_str());
    unsigned int floorSpecularMap = specularMap;
    vector<glm::vec3> aliens
            {
                    glm::vec3(-25.5f, 5.0f, -12.0f),//nazad
                    glm::vec3( -18.5f, 5.0f, -16.0f),
                    glm::vec3( -7.0f, 5.0f, -19.0f),
                    glm::vec3(0.0f, 5.0f, -5.0f),
                    glm::vec3 (7.0f, 5.0f, -22.0f),
                    glm::vec3(-20.0f, 5.0f, -17.5f),
                    glm::vec3( -12.5f, 5.0f, -11.5f),
                    glm::vec3( -3.0f, 5.0f, -2.5f),
                    glm::vec3(2.5f, 5.0f, -10.5f),
                    glm::vec3 (15.5f, 5.0f, -2.5f),
                    glm::vec3(-25.5f, 5.0f, 16.0f),//napred
                    glm::vec3( -15.5f, 5.0f, 8.0f),
    ...
    Model ship(FileSystem::getPath("resources/objects/vanze/Low_poly_UFO.obj"));
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if(ssaoButton==true){
            ...
            // point light setup
            lightShader.setVec3("pointLight.position", location);
            ...
            lightShader.setVec3("spotLight.ambient", 0.0 ,(sin(15 * time) + 1) / 2.0, 0.0f);
            lightShader.setVec3("spotLight.diffuse", 0.0f, 0.0f, 0.0f);
            lightShader.setVec3("spotLight.specular", 0.0, (sin(15 * time) + 1) / 2.0, 0.0f);
            ...
            for (unsigned int i = 0; i < aliens.size(); i++) {
                r=20.0f;
                float x=location.x+r*cos(glm::radians(360.0/p*i));
                float y=3.0f;
                float z=location.z+r*sin(glm::radians(360.0/p*i));
                model = glm::mat4(1.0f);
